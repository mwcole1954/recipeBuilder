...
class DragDrop:
            ...
            if t_row is not None and 0 <= t_row < len(self.label_list) and 0 <= w_row < len(self.label_list):
                if isinstance(widget, Text) or isinstance(widget, Label):
                    if t_row < w_row:
                        ...
                        self.label_list.pop(w_row)
                        self.value_list.pop(w_row)
                # update the changes to the data_list
                # TODO How to reset the panel list to the last updated row
    ...
    def get_drag_scroll(self, event):
        # current y coordinate of the mouse in the window
        y = self.sf.c.winfo_pointery()
        # current root-y coordinate of the window
        root_y = self.sf.c.winfo_rooty()
        # exact window pos_x = self.sf.c.winfo_pointery() - self.sf.c.winfo_rooty()
        # subtract the root-y offset from the point-y
        # the + 1 offsets some motion in the list while scrolling from bottom past the top
        # without the offset, the top of the list drops down a few pixels
        exact_y = y - root_y + 1

        if exact_y / int(self.sf.c["height"]) >= self.sf.c.yview()[1]:
            self.sf.c.yview_scroll(1, "units")
        elif exact_y / int(self.sf.c["height"]) <= self.sf.c.yview()[0]:
            self.sf.c.yview_scroll(-1, "units")
